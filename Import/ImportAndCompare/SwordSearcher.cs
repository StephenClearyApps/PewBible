...
public static class SwordSearcher
{
    public static List<Verse> Import(IReadOnlyList<string> text)
    {
        ...
        foreach (var line in text.Where(x => !string.IsNullOrEmpty(x)))
        {
            ...
            result.Add(new Verse(currentBook ?? throw new InvalidCastException("No book for chapter"), currentChapter, verseNumber, verseText.Replace("--", "—").Replace("LORD'S", "LORD's")));
        }
