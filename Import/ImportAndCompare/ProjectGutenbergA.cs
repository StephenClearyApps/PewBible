    ...
    public static List<Verse> Import(IReadOnlyList<string> text)
    {
        ...
        foreach (var line in SplitVerses(CombineMultilineVerses(text)))
        {
            ...
            else
            {
                ...
                verse = verse.Replace(":", ": ");
                result.Add(new Verse(currentBook ?? throw new InvalidCastException("No book for chapter"), int.Parse(match.Groups[1].Value), int.Parse(match.Groups[2].Value), verse));
            }
        }
        return result;
    }
