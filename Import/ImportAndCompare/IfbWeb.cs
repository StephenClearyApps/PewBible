    ...
    public static List<Verse> Import(IReadOnlyList<string> text)
    {
        ...
        foreach (var line in text.Where(x => !string.IsNullOrEmpty(x)).ConvertPsalmToChapters().MergePreColophones())
        {
            ...
            else
            {
                ...
                result.Add(new Verse(currentBook ?? throw new InvalidCastException("No book for chapter"), currentChapter, int.Parse(line.Substring(0, spaceIndex)), line.Substring(spaceIndex + 1).Replace('#', ' ').Replace('[', ' ').Replace(']', ' ')));
            }
        }
